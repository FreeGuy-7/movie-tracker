...
def render_dashboard(user: dict, active: list[dict], all_users: list[dict], notice: str = "") -> bytes:
    ...
    users_html = ""
    if user.get("role") == "admin":
        user_rows = "".join(f"<tr><td>{escape(item['username'])}</td><td>{escape(item.get('role', 'user'))}</td><td>{escape(item.get('discord_user_id', '') or 'Not set')}</td><td>{sum(1 for trigger in active if trigger.get('owner_id') == item['id'])}/{MAX_ACTIVE_TRIGGERS}</td></tr>" for item in all_users)
