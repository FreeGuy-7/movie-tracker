...
def render_auth_page(signup: bool, error: str = "", values: dict[str, str] | None = None) -> bytes:
    values = values or {}
    if signup:
        username = escape(values.get("username", ""))
        discord_user_id = escape(values.get("discord_user_id", ""))
        body = f"<section class=card><h1>Create account</h1><p class=muted>Use the access key shared by the administrator.</p><p class=error id=signup-error>{escape(error)}</p><form method=post action='/signup' class=grid id=signup-form onsubmit='return validateSignup()'><label>Username<input required name=username autocomplete=username value='{username}'></label><label>Discord user ID<input required name=discord_user_id inputmode=numeric placeholder='Numeric ID for new-show tags' value='{discord_user_id}'></label><label>Password<input required minlength=8 type=password name=password autocomplete=new-password><small>Use at least 8 characters.</small></label><label>Confirm password<input required minlength=8 type=password name=confirm_password autocomplete=new-password></label><label class=wide>Access key<input required type=password name=access_key></label><div><button>Sign up</button> <a href='/login'>Back to login</a></div></form><script>function validateSignup(){{const form=document.getElementById('signup-form');const password=form.password.value;const confirmation=form.confirm_password.value;const error=document.getElementById('signup-error');if(password.length < 8){{error.textContent='Password must be at least 8 characters.';form.password.focus();return false}}if(password !== confirmation){{error.textContent='Passwords do not match.';form.confirm_password.focus();return false}}error.textContent='';return true}}</script></section>"
    else:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:
        ...
        if path == "/signup":
            ...
            if error:
                self.send_html(render_auth_page(True, error, {"username": username, "discord_user_id": self.value(data, "discord_user_id")}), HTTPStatus.BAD_REQUEST)
                return
